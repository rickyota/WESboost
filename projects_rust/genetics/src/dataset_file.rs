...
pub struct DatasetFile {
    ...
    fin_group_snv: Option<PathBuf>,
    fin_agg_snv: Option<PathBuf>,
    ...
    group_snv_buf: Option<Vec<u8>>,
    agg_snv_buf: Option<Vec<u8>>,
...
impl DatasetFile {
    pub fn check_valid_fin(&self) {
        ...
        self.fins_genot().check_valid_open();
    }
    ...
    pub fn update_file_group_snv(&mut self, fin_group_snv: Option<PathBuf>) {
        self.fin_group_snv = fin_group_snv;
    }
    ...
    pub fn update_file_agg_snv(&mut self, fin_snv_agg: Option<PathBuf>) {
        self.fin_agg_snv = fin_snv_agg;
    }

    pub fn fins_genot(&self) -> &GenotFiles {
        &self.fins_genot
    }
    ...
    pub fn fin_group_snv(&self) -> Option<&Path> {
        self.fin_group_snv.as_ref().map(|x| x.as_path())
    }
    pub fn fin_agg_snv(&self) -> Option<&Path> {
        self.fin_agg_snv.as_ref().map(|x| x.as_path())
    }
    ...
    pub fn agg_snv_buf(&self) -> Option<&[u8]> {
        self.agg_snv_buf.as_deref()
    }
    pub fn group_snv_buf(&self) -> Option<&[u8]> {
        self.group_snv_buf.as_deref()
    }
    ...
    fn read_group_snv(&mut self) {
        if self.group_snv_buf.is_none() {
            if let Some(fin_snv_set) = self.fin_group_snv() {
                let snv_set_buf =
                    textfile::read_file_to_end(fin_snv_set, None).unwrap_or_else(|_| {
                        panic!("Cannot read file: {:?}", fin_snv_set);
                    });
                self.group_snv_buf = Some(snv_set_buf);
            }
        }
        // otherwise, do nothing
    }

    fn read_agg_snv(&mut self) {
        if self.agg_snv_buf.is_none() {
            if let Some(fin_agg_snv) = self.fin_agg_snv() {
                let agg_snv_buf =
                    textfile::read_file_to_end(fin_agg_snv, None).unwrap_or_else(|_| {
                        panic!("Cannot read file: {:?}", fin_agg_snv);
                    });
                self.agg_snv_buf = Some(agg_snv_buf);
            }
        }
        // otherwise, do nothing
    }
