...
pub fn load_wgts_file(fwgt: &Path, is_nonadd: bool) -> Vec<Wgt> {
    ...
    for col_i in 0..col_n {
        ...
        let col_name: Option<String> = match col.as_str() {
            "vid" | "sid" | "rs" | "sida" | "id" | "groupid" => Some("var".to_string()),
            "chrom" | "chr" => Some("chrom".to_string()),
            "pos" | "position" | "pos_start" => Some("pos".to_string()),
            "pos_end" => Some("pos_end".to_string()),
            ...
            "a1_frq" | "freq" => Some("a1_frq".to_string()),
            "a2_frq" => Some("a2_frq".to_string()),
            "snvs" => Some("group_ids".to_string()),
            _ => None,
            //z => z, // unuse col
    ...
    } else if col_to_i.contains_key("group_ids") {
        vec!["var", "chrom", "pos", "pos_end", "group_ids"]
    } else {
    ...
    for wgt_i in 1..vss[0].len() {
        // for snv
        let snv = if col_to_i.contains_key("pos_end") {
            // let group_ids = vss[col_to_i["group_ids"]][wgt_i].split(",").collect();
            SnvId::new_group(
                vss[col_to_i["var"]][wgt_i].clone(),
                vss[col_to_i["chrom"]][wgt_i].clone(),
                &vss[col_to_i["pos"]][wgt_i],
                &vss[col_to_i["pos_end"]][wgt_i],
                vss[col_to_i["group_ids"]][wgt_i].split(",").collect(), // group_ids,
            )
        } else {
            SnvId::new(
                vss[col_to_i["var"]][wgt_i].clone(),
        ...
        let maf = if !col_to_i.contains_key("a1_frq") && !col_to_i.contains_key("a2_frq") {
            None
