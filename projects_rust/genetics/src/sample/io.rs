...
pub fn samples_id_use_samples(sample_buf: Option<&[u8]>, fins_genot: &GenotFiles) -> Vec<String> {
    if sample_buf.is_none() {
        return genot_io::load_samples_id(fins_genot, None);
    }

    let (use_sample, _) = make_use_samples_buf(sample_buf, fins_genot);

    //return genot_io::load_samples_id(fin_genot, None);
    genot_io::load_samples_id(fins_genot, Some(&use_sample))
}
...
pub fn make_use_samples_buf(
    ...
    let samples_in: Vec<String> = genot_io::load_samples_id(fins_genot, None);
