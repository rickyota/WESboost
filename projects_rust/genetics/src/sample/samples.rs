...
use crate::{genot_io, vec, GenotFiles, B8};
...
impl Samples {
    pub fn new_plink(
        ...
        let samples_id = genot_io::load_samples_id(fins_genot, use_samples);
        ...
        let ys: Option<Vec<bool>> = if load_y {
            genot_io::load_ys_buf(fins_genot, phe_buf, phe_name, &sample_id_to_n)
        } else {
            None
        };

        let covs: Option<Covs> =
            cov_name.map(|x| Covs::new_buf(phe_buf, fins_genot, x, &sample_id_to_n));
...
pub fn create_sample_id_to_n(
    ...
    let samples_id: Vec<String> = genot_io::load_samples_id(fins_genot, use_samples);
