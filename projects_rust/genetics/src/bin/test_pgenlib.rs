...
fn main() {
    ...
    let g_plink2 = genetics::test_pgenlib(
        &GenotFiles::new_plink2vzs(vec![fin.clone()]),
        fin_sample.as_deref(),
    );
    let g_plink1 = genetics::test_pgenlib(
        &GenotFiles::new_plink1(vec![fin.clone()]),
        fin_sample.as_deref(),
    );
