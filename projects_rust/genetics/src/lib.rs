...
pub use arg::GenotFormatArg;
pub use cov::CovKind;
pub use dataset::{Dataset, FillMissing, FillMissingGroup, FillMissingScore};
...
pub use genot_io::{GenotFile, GenotFiles};
...
pub fn run_score(
    ...
    // fill_missing_group=FillMissingGroup::Ref in score
...
pub fn run_group_freq(
    fgroup_snv_freq: &Path,
    // fgroup_snv: &Path,
    dfile: &DatasetFile,
    mem: Option<usize>,
) {
    ...
    // let (dataset, _) = load_dataset::load_dataset(dfile, Some(FillMissing::Ref), false, mem);

    let dataset = Dataset::new_datasetfile(
        dfile,
        false,
        None,
        None,
    ...
    aggregation::group_freq(fgroup_snv_freq, &dataset);
    // aggregation::group_freq(fgroup_snv_freq, fgroup_snv, &dataset);
}

pub fn test_pgenlib(fin_genot: &GenotFiles, _fin_sample: Option<&Path>) -> Genot {
    let (snvs_in, file_snv_allele_idx_in) = genot_io::load_snvs(fin_genot);
    ...
    log::debug!("m_in: {}", m_in);
    let n_in: usize = genot_io::compute_num_sample(fin_genot);
    log::debug!("n_in: {}", n_in);
