...
use super::{GenotMut, GenotRef, GenotSnvMut, GenotSnvRef};
...
pub trait BaseGenot {
    ...
    // TODO:
    //fn confirmed_no_missing(&self) -> bool;
    ...
    fn split_genot(&self, m: usize) -> (GenotRef, GenotRef) {
        let (cm1, cm2) = self.genot_inner().split_rows(m);
        (GenotRef::new(cm1), GenotRef::new(cm2))
    }
}
    ...
    Self::Inner: BaseCMatrixMut,
{
    ...
    fn split_genot_mut(&mut self, m: usize) -> (GenotMut, GenotMut) {
        let (cm1, cm2) = self.genot_inner_mut().split_rows_mut(m);
    ...
    Self::Inner: BaseCVec,
{
    ...
    fn maf_group(&self) -> f64 {
        ...
        let maf = (c1 as f64) / (cnomissing as f64);
    ...
    // TODO: stat_contingency_table_no_missing()

    // TODO: add n_case: Option<usize> in argment to make it faster
    ...
    Self::Inner: BaseCVecMut,
{
    ...
    // missing -> ref/ref
    fn fill_missing_ref(&mut self) {
        let n = self.n();

        for ni in 0..n {
            if self.get_val_unchecked(ni) == 3 {
                self.set_unchecked(0, ni);
            }
        }
    }
