...
// TODO
// confirmed_no_missing: bool; if true, no missing values in the data.
//                                                   if false, data could have missing values.
#[derive(Debug, PartialEq, Eq, Clone)]
//pub struct Genot(CMatrix, bool);
pub struct Genot(CMatrix);

#[derive(Debug, PartialEq, Eq)]
pub struct GenotRef<'a>(CMatrixRef<'a>);
...
impl<'a> BaseGenot for GenotRef<'a> {
    type Inner = CMatrixRef<'a>;
    fn genot_inner(&self) -> &Self::Inner {
        &self.0
    }
}
...
impl<'a> GenotRef<'a> {
    pub fn new(cmat_ref: CMatrixRef) -> GenotRef {
        GenotRef(cmat_ref)
    }
}
