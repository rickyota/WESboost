...
use super::{CBitsRef, CVecMut, CVecRef, B8};
use super::{CMatrix, CMatrixMut, CMatrixRef};
...
pub trait BaseCMatrix {
    ...
    //fn clone_to_cmatrix(&self) -> CMatrix;
    fn clone_to_cmatrix(&self) -> CMatrix {
        let m = CMatrix::new_vec_inner(
            self.row_n(),
            self.col_n(),
            self.digit_n(),
            self.inner().to_vec(),
    ...
    fn as_cbits_ref(&self, row_i: usize, digit_i: usize) -> CBitsRef {
        CBitsRef::new(self.inner_col_digit(row_i, digit_i), self.col_n())
    }
    ...
    fn split_rows(&self, row_i: usize) -> (CMatrixRef, CMatrixRef) {
        ...
        let (inner0, inner1) = self.inner().split_at(index_split);
        let mut0 = CMatrixRef::new(inner0, row_n0, col_n, digit_n);
        let mut1 = CMatrixRef::new(inner1, row_n1, col_n, digit_n);
...
pub trait BaseCMatrixMut: BaseCMatrix {
    ...
    #[inline]
    fn set_bool_unchecked_true(&mut self, row_i: usize, col_i: usize, digit_i: usize) {
        self.inner_col_digit_mut(row_i, digit_i)[col_i >> 3] |= 0x80 >> (col_i & 7);
    }

    #[inline]
    fn set_bool_unchecked_false(&mut self, row_i: usize, col_i: usize, digit_i: usize) {
        self.inner_col_digit_mut(row_i, digit_i)[col_i >> 3] &= !(0x80 >> (col_i & 7));
    }
    ...
    fn set_bool_unchecked(&mut self, b: bool, row_i: usize, col_i: usize, digit_i: usize) {
        match b {
            true => {
                self.set_bool_unchecked_true(row_i, col_i, digit_i)
                //self.inner_col_digit_mut(row_i, digit_i)[col_i >> 3] |= 0x80 >> (col_i & 7);
            }
            false => {
                self.set_bool_unchecked_false(row_i, col_i, digit_i)
                //self.inner_col_digit_mut(row_i, digit_i)[col_i >> 3] &= !(0x80 >> (col_i & 7));
            }
        }
    }
    ...
    /// now val -> (bool, bool) is 2(10)->(1,1), 1(01)->(1,0), 0(00)->(0,0), 3(11)->(0,1)
    // TODO: genotype encoding should be 2=(1,1), 1=(1,0), 0=(0,1), m=(0,0) -> since (0,0) could be mixed with padding
    // -> not necessary. padding should be treated separately.
    #[inline]
    ...
    fn set_by_bool_init_unchecked(&mut self, b0: bool, b1: bool, row_i: usize, col_i: usize) {
        if b0 == true {
            self.set_bool_unchecked_true(row_i, col_i, 0);
            //self.set_bool_unchecked(b0, row_i, col_i, 0);
        }
        if b1 == true {
            self.set_bool_unchecked_true(row_i, col_i, 1);
            //self.set_bool_unchecked(b1, row_i, col_i, 1);
        }
    }
    ...
    fn split_rows_mut(&mut self, row_i: usize) -> (CMatrixMut, CMatrixMut) {
        let (row_n, col_n, digit_n) = (self.row_n(), self.col_n(), self.digit_n());
        let (row_n0, row_n1) = (row_i, row_n - row_i);
        let index_split = self.index_row_unchecked(row_i);
