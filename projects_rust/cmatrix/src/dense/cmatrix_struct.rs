...
impl CMatrix {
    ...
    pub fn new_vec_inner(row: usize, col: usize, digit: usize, vec: Vec<B8>) -> CMatrix {
        CMatrix::check_new(row, col, digit);
        let m = CMatrix {
            inner: vec,
            row_n: row,
...
mod tests {
    ...
    #[test]
    fn test_clone_to_cmatrix() {
        let m = setup1();
        let cm = m.clone_to_cmatrix();
        assert_eq!(cm, m);
    }
    ...
    #[test]
    fn test_set_unckeccked() {
        let mut m = setup0();
        m.set_unchecked(0, 1, 2);
        assert_eq!(m.get_val(1, 2), 0);
        m.set_unchecked(1, 1, 2);
        assert_eq!(m.get_val(1, 2), 1);
        m.set_unchecked(2, 1, 2);
        assert_eq!(m.get_val(1, 2), 2);
        m.set_unchecked(3, 1, 2);
        assert_eq!(m.get_val(1, 2), 3);
        m.set_unchecked(0, 1, 2);
        assert_eq!(m.get_val(1, 2), 0);
    }
