...
impl DoutFile {
    ...
    pub fn dout_agg_para(&self) -> DoutParaFile {
        let mut d = self.dout.to_owned();
        let dpara = String::from("para");
        d.push(dpara);

        DoutParaFile::new(d)
    }

    pub fn dout_loss_for_prune(&self) -> DoutParaFile {
        // TODO: use .join
        let mut d = self.dout.to_owned();
        let mut dpara = String::from("loss-for-prune");
        d.push(dpara);
...
impl DoutParaFile {
    ...
    pub fn fgroup_loss(&self) -> PathBuf {
        let mut f = self.dout_para().to_owned();
        f.push("loss/");
        let fname = "group.loss";
        f.push(fname);
        f
    }
    pub fn bufwriter_fgroup_loss(&self) -> BufWriter<File> {
        let fgroup_loss = self.fgroup_loss();
        create_dir(&fgroup_loss.parent().unwrap());
        textfile::bufwriter(&fgroup_loss)
    }

    pub fn fgroup_whole_loss(&self) -> PathBuf {
        let mut f = self.dout_para().to_owned();
        f.push("loss/");
        let fname = "group.whole.loss";
        f.push(fname);
        f
    }
    pub fn bufwriter_fgroup_whole_loss(&self) -> BufWriter<File> {
        let fgroup_loss = self.fgroup_whole_loss();
        create_dir(&fgroup_loss.parent().unwrap());
        textfile::bufwriter(&fgroup_loss)
    }

    pub fn fagg_loss_min(&self, agg_id: &str) -> PathBuf {
        let mut f = self.dout_para().to_owned();
        f.push("loss/min_range/");
        let fname = "agg-".to_string() + agg_id + ".loss";
        f.push(fname);
        f
    }
    pub fn bufwriter_fagg_loss_min(&self, agg_id: &str) -> BufWriter<File> {
        let fagg_loss = self.fagg_loss_min(agg_id);
        create_dir(&fagg_loss.parent().unwrap());
        textfile::bufwriter(&fagg_loss)
    }
}
