...
pub enum LossStruct {
    ...
    // TODO: Vec<Vec<f64>>: m x 2
    // NG: create Vec<(f64,f64)> => cannot access by loss[mi][si]

    // ModelFree or Logit
    LossOne(Vec<f64>),
    // TODO: m should not be necessary
    // loss, m
...
impl LossStruct {
    pub fn inner_mut(&mut self) -> &mut [f64] {
        match self {
            ...
            LossStruct::LossOne(ref mut v) => v,
    ...
    pub fn new(boost_type: BoostType, m: usize) -> Self {
        match boost_type {
            ...
            | BoostType::LogitCommon => LossStruct::LossOne(vec![f64::NAN; m]),
    ...
    pub fn new_vec(boost_type: BoostType, v: Vec<f64>) -> Self {
        ...
        match boost_type {
            ...
            | BoostType::LogitAdd
            | BoostType::LogitMhcNoMissing
            | BoostType::LogitCommon => LossStruct::LossOne(v),
            _ => unimplemented!(),
        }
    }
    ...
    fn search_topprop_modelfree(&self, prop: f64) -> Vec<bool> {
        if let LossStruct::LossOne(loss) = self {
            let m = loss.len();
    ...
    pub fn search_topprop_n(
        ...
        match self {
            LossStruct::ConstAda(..) => unimplemented!(),
            LossStruct::LossOne(..) => self.search_topprop_modelfree_n(m_top, extract_snvs),
            LossStruct::Interaction(..) => self.search_topprop_modelfree_n(m_top, extract_snvs),
        }
    }
    ...
    pub fn search_topprop_interaction_n(
        ...
        match self {
            ...
            LossStruct::Interaction(..) => {
                self.search_topprop_n_interaction(m_top)
    ...
    fn search_topprop_modelfree_n(
        ...
        //if let LossStruct::LossOne(loss, _)  = self {
        match self {
            LossStruct::LossOne(loss) | LossStruct::Interaction(loss, ..) => {
    ...
    pub fn decrease_by(&self, loss_2: &Self) -> Self {
        match self {
            ...
            LossStruct::LossOne(loss) => {
                if let LossStruct::LossOne(loss_2) = loss_2 {
...
pub fn search_min_loss_gt_interaction(
    ...
    if loss_min_single < loss_min_interaction {
        ...
        let wgt_min = Wgt::new_snv_id_freemodel(
            ...
            var_mi_single,
            snvs.freqs_a1().unwrap()[var_mi_single],
        );
        ...
        let wgt_min = Wgt::new_snv_id_interaction(
            ...
            var_mi_1,
            var_mi_2,
            snvs.freqs_a1().unwrap()[var_mi_1],
            snvs.freqs_a1().unwrap()[var_mi_2],
        );
        let wgt_boost_min = WgtBoost::new_wgt_loss(wgt_min, iteration, Some(loss_min_single));
        wgt_boost_min
    }
