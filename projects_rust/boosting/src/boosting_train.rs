...
fn create_initial_interaction(
    ...
    let mut alphas_single = vec![f64::NAN; dataset.snvs().snvs_both_n()];
    ...
    let m = dataset.snvs().snvs_both_n();
...
pub fn boosting_batch<W: std::io::Write>(
    ...
    let n = dataset.samples().samples_n();
    let n_val = dataset_val.map_or(0, |val| val.samples().samples_n());
...
fn set_wgt_m<W: WgtTrait>(wgts: &mut [W], snvs: &Snvs) {
    ...
    let mut sida_to_m = HashMap::with_capacity(snvs.snvs_both_n());
