//! Application of **WESboost** for research.
//! Input plink file to run WESboost.
//!
//! changed inputs;
//!     - GenotFormatArg
//!     - remove fin_cov
...
use genetics::{DatasetFile, FillMissingGroup, FillMissingScore, GenotFormatArg};
...
struct ScoreArgs {
    ...
    use_snv_pos: bool,
    #[arg(long, help = "mode, mean, ref; How to treat missing values.")]
    ...
    #[arg(long, help = "ref; How to treat missing values.")]
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Score(args) => {
            ...
            let mut dfile =
                DatasetFile::new(fins_genot, fin_phe, None, cov_name, None, fin_sample, None);
            dfile.reads();
            let dfile = dfile;
            dfile.check_valid_fin();
            ...
            let missing_score = args
                .missing
                .map(|x| FillMissingScore::from_str(&x).unwrap());

            let fill_missing_group = args
                .missing_group
                .map(|x| FillMissingGroup::from_str(&x).unwrap());
