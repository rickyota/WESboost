...
RPgenReader::RPgenReader() : _info_ptr(nullptr), _allele_idx_offsetsp(nullptr), _nonref_flagsp(nullptr) {}
// RPgenReader::RPgenReader() : _info_ptr(nullptr), _allele_idx_offsetsp(nullptr), _nonref_flagsp(nullptr), _state_ptr(nullptr) {}

// void RPgenReader::Load(String filename, Nullable<List> pvar, Nullable<int> raw_sample_ct, Nullable<IntegerVector> sample_subset_1based) {
void RPgenReader::Load(std::string filename, RPvar *pvar, uint32_t cur_sample_ct, std::vector<int> sample_subset_1based, int nthr) {
    if(_info_ptr) {
        Close();
    }
    ...
    if(PgfiInitPhase1(fname, nullptr, cur_variant_ct, cur_sample_ct, &header_ctrl, _info_ptr, &pgfi_alloc_cacheline_ct, errstr_buf) !=
    ...
    if(pvar->GetVariantCt() != raw_variant_ct) {
        fprintf(stderr, "pvar and pgen have different variant counts");
        exit(-1);
        // stop("pvar and pgen have different variant counts");
    }
    _allele_idx_offsetsp = pvar->GetAlleleIdxOffsetsp();
    //_allele_idx_offsetsp = rp->GetAlleleIdxOffsetsp();
    if(_allele_idx_offsetsp) {
        _info_ptr->allele_idx_offsets = _allele_idx_offsetsp->p;
    }
    _info_ptr->max_allele_ct = pvar->GetMaxAlleleCt();
    ...
    _pgv.resize(nthr);
    _transpose_batch_buf.resize(nthr);
    _multivar_vmaj_geno_buf.resize(nthr);
    _multivar_vmaj_phasepresent_buf.resize(nthr);
    _multivar_vmaj_phaseinfo_buf.resize(nthr);
    _multivar_smaj_geno_batch_buf.resize(nthr);
    _multivar_smaj_phaseinfo_batch_buf.resize(nthr);
    _multivar_smaj_phasepresent_batch_buf.resize(nthr);
    ...
    for(int i = 0; i < nthr; i++) {
        ...
        if(multiallelic_hc_byte_ct) {
            _pgv[i]->patch_01_set = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
            pgr_alloc_iter = &(pgr_alloc_iter[sample_subset_byte_ct]);
            _pgv[i]->patch_01_vals = reinterpret_cast<plink2::AlleleCode *>(pgr_alloc_iter);
            pgr_alloc_iter = &(pgr_alloc_iter[ac_byte_ct]);
            _pgv[i]->patch_10_set = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
            pgr_alloc_iter = &(pgr_alloc_iter[sample_subset_byte_ct]);
            _pgv[i]->patch_10_vals = reinterpret_cast<plink2::AlleleCode *>(pgr_alloc_iter);
        ...
        // printf("ac\n");
        // fflush(stdout);
        _transpose_batch_buf[i] = reinterpret_cast<plink2::VecW *>(pgr_alloc_iter);
        pgr_alloc_iter = &(pgr_alloc_iter[plink2::kPglBitTransposeBufbytes]);
        _multivar_vmaj_geno_buf[i] = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
        pgr_alloc_iter = &(pgr_alloc_iter[plink2::kPglNypTransposeBatch * genovec_byte_ct]);
        _multivar_vmaj_phasepresent_buf[i] = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
        pgr_alloc_iter = &(pgr_alloc_iter[plink2::kPglNypTransposeBatch * sample_subset_byte_ct]);
        _multivar_vmaj_phaseinfo_buf[i] = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
        pgr_alloc_iter = &(pgr_alloc_iter[plink2::kPglNypTransposeBatch * sample_subset_byte_ct]);
        _multivar_smaj_geno_batch_buf[i] = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
        pgr_alloc_iter = &(pgr_alloc_iter[plink2::kPglNypTransposeBatch * plink2::kPglNypTransposeBatch / 4]);
        _multivar_smaj_phaseinfo_batch_buf[i] = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
        pgr_alloc_iter = &(pgr_alloc_iter[plink2::kPglNypTransposeBatch * plink2::kPglNypTransposeBatch / 8]);
        _multivar_smaj_phasepresent_batch_buf[i] = reinterpret_cast<uintptr_t *>(pgr_alloc_iter);
...
// }
//
// uint32_t RPgenReader::GetSubsetSize() const { return _subset_size; }
//
...
void RPgenReader::ReadIntHardcalls(int *buf, size_t const &n, int const &thr, int variant_idx, int allele_idx) {
    if(!_info_ptr) {
    ...
    plink2::GenoarrLookup256x4bx4(_pgv[thr]->genovec, kGenoRInt32Quads, _subset_size[thr], &buf[0]);
}

// missing = -3.0 -> to modify
...
void RPgenReader::ReadHardcalls(double *buf, size_t const &n, int const &thr, int variant_idx, int allele_idx) {
    if(!_info_ptr) {
        fprintf(stderr, "pgen is closed");
        exit(-1);
    }
    ...
    plink2::GenoarrLookup16x8bx2(_pgv[thr]->genovec, kGenoRDoublePairs, _subset_size[thr], &buf[0]);
}

// this can be run in parallel
// void RPgenReader::Read(NumericVector buf, int variant_idx, int allele_idx) {
void RPgenReader::Read(double *buf, size_t const &n, int const &thr, int variant_idx, int allele_idx) {
    if(!_info_ptr) {
        fprintf(stderr, "pgen is closed");
        exit(-1);
    }
...
//         }
//         uint32_t dosage_ct;
//         plink2::PglErr reterr = PgrGetD(_subset_include_vec, _subset_index, _subset_size, variant_idx, _state_ptr, _pgv.genovec,
//         _pgv.dosage_present,
//                                         _pgv.dosage_main, &dosage_ct);
//         if(reterr != plink2::kPglRetSuccess) {
//             char errstr_buf[256];
...
//         }
//         uint32_t dosage_ct;
//         plink2::PglErr reterr = plink2::PgrGetD(_subset_include_vec, _subset_index, _subset_size, variant_idx, _state_ptr, _pgv.genovec,
//                                                 _pgv.dosage_present, _pgv.dosage_main, &dosage_ct);
//         if(reterr != plink2::kPglRetSuccess) {
//             char errstr_buf[256];
//             snprintf(errstr_buf, 256, "PgrGetD() error %d", static_cast<int>(reterr));
//             stop(errstr_buf);
...
void RPgenReader::SetSampleSubsetInternal(std::vector<int> &sample_subset_1based, int const &thr) {
