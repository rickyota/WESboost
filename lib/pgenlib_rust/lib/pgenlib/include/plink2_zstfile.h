...
ENUM_U31_DEF_START()
kFileUncompressed, kFileGzip, kFileBgzf, kFileZstd ENUM_U31_DEF_END(FileCompressionType);
...
HEADER_INLINE int32_t ZstRfileIsOpen(const zstRFILE *zrf_ptr) { return (GET_PRIVATE(*zrf_ptr, m).ff != nullptr); }

HEADER_INLINE int32_t zsteof(const zstRFILE *zrf_ptr) { return (GET_PRIVATE(*zrf_ptr, m).reterr == kPglRetEof); }

HEADER_INLINE const char *zsterror(const zstRFILE *zrf_ptr) { return GET_PRIVATE(*zrf_ptr, m).errmsg; }
