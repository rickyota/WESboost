...
void GenoarrPhasedToAlleleCodes(const uint64_t *genoarr_to_intcode_dpair_table, const uintptr_t *genoarr, const uintptr_t *phasepresent,
...
void GenoarrMPToAlleleCodes(const uint64_t *geno_to_intcode_dpair_table, const PgenVariant *pgv, uint32_t sample_ct, unsigned char *phasebytes,
    ...
    if((!patch_01_ct) && (!patch_10_ct)) {
        GenoarrPhasedToAlleleCodes(geno_to_intcode_dpair_table, genoarr, phasepresent, phaseinfo, sample_ct, phasepresent_ct, phasebytes,
                                   allele_codes);
        return;
    }
