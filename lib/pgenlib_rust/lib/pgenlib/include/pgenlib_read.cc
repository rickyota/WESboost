...
PglErr PgfiInitPhase1(const char *fname, const char *pgi_fname, uint32_t raw_variant_ct, uint32_t raw_sample_ct, PgenHeaderCtrl *header_ctrl_ptr,
    ...
    *header_ctrl_ptr = 0;
        ...
        if(raw_variant_ct == UINT32_MAX) {
            ...
            if(unlikely((quotient > kPglMaxSampleCt) || (quotient * const_vrec_width + 3 != fsize))) {
                snprintf(errstr_buf, kPglErrstrBufBlen,
                         ...
                         "quotient should be smaller than 2^31 - 1).\n",
    ...
    if(unlikely((!fread_unlocked(&(pgfip->raw_variant_ct), sizeof(int32_t), 1, header_ff)) ||
                (!fread_unlocked(&(pgfip->raw_sample_ct), sizeof(int32_t), 1, header_ff)) || (!fread_unlocked(header_ctrl_ptr, 1, 1, header_ff)))) {
        snprintf(errstr_buf, kPglErrstrBufBlen, "Error: %s read failure: %s.\n", header_fname, strerror(errno));
        return kPglRetReadFail;
    }
...
void FillPgenReadErrstrFromNzErrno(char *errstr_buf) { return FillPgenHeaderReadErrstrFromNzErrno(0, errstr_buf); }
...
void FillPgenReadErrstrFromErrno(char *errstr_buf) { return FillPgenHeaderReadErrstrFromErrno(0, errstr_buf); }
...
void FillPgenReadErrstr(FILE *ff, char *errstr_buf) { FillPgenHeaderReadErrstr(ff, 0, errstr_buf); }
...
PglErr PgfiInitPhase2Ex(PgenHeaderCtrl header_ctrl, uint32_t allele_cts_already_loaded, uint32_t nonref_flags_already_loaded, uint32_t use_blockload,
                        uint32_t vblock_idx_start, uint32_t vidx_end, uint32_t *max_vrec_width_ptr, PgenFileInfo *pgfip, unsigned char *pgfi_alloc,
                        PgenExtensionLl *header_exts, PgenExtensionLl *footer_exts, uintptr_t *pgr_alloc_cacheline_ct_ptr, char *errstr_buf) {
...
PglErr PgfiInitReloadExtSet(uint32_t is_pgi, FILE *ff, uintptr_t *ext_bitarr, uint32_t *ext_bitarr_cumulative_popcounts, uint64_t *footer_fpos_ptr,
                            uint32_t *word_ct_ptr, char *errstr_buf) {
...
PglErr PgfiInitFillExts(const uintptr_t *ext_bitarr, const uint32_t *ext_cumulative_popcounts, uint32_t word_ct, uint32_t is_pgi, FILE *ff,
                        PgenExtensionLl *exts, char *errstr_buf) {
    {
...
PglErr PgfiInitLoadExts(PgenHeaderCtrl header_ctrl, PgenFileInfo *pgfip, PgenExtensionLl *header_exts, PgenExtensionLl *footer_exts,
                        char *errstr_buf) {
    ...
    PglErr reterr =
        PgfiInitReloadExtSet(is_pgi, header_ff, header_bitarr, header_bitarr_cumulative_popcounts, nullptr, &header_ext_word_ct, errstr_buf);
    if(unlikely(reterr)) {
        return reterr;
    }
    ...
    reterr = PgfiInitReloadExtSet(is_pgi, header_ff, footer_bitarr, footer_bitarr_cumulative_popcounts, &footer_extensions_fpos, &footer_ext_word_ct,
                                  errstr_buf);
    if(unlikely(reterr)) {
        return reterr;
    }
    ...
    if(footer_exts) {
        ...
        reterr =
            PgfiInitFillExts(footer_bitarr, footer_bitarr_cumulative_popcounts, footer_ext_word_ct, 0, pgfip->shared_ff, footer_exts, errstr_buf);
        if(unlikely(reterr)) {
            return reterr;
        }
    }
...
PglErr PgrInit(const char *fname, uint32_t max_vrec_width, PgenFileInfo *pgfip, PgenReader *pgr_ptr, unsigned char *pgr_alloc) {
    ...
    if((gflags & kfPgenGlobalDifflistOrLdPresent) || (max_allele_ct > 2)) {
        pgrp->workspace_difflist_sample_ids =
            S_CAST(uint32_t *, arena_alloc_raw_rd((max_difflist_entry_ct_base + 1) * sizeof(int32_t), &pgr_alloc_iter));
    }
    if(gflags & kfPgenGlobalDifflistOrLdPresent) {
        // const uint32_t max_difflist_entry_ct = max_difflist_entry_ct_base * (1 + ld_compression_present);
        ...
        if(ld_compression_present) {
            ...
            pgrp->ldbase_difflist_sample_ids =
                S_CAST(uint32_t *, arena_alloc_raw_rd((max_difflist_entry_ct_base + 1) * sizeof(int32_t), &pgr_alloc_iter));
        }
    } else {
...
PglErr ParseOnebitUnsafe(const unsigned char *fread_end, const unsigned char **fread_pp, PgenReaderMain *pgrp, uintptr_t *__restrict genoarr) {
    ...
    for(;; ++genoarr_widx) {
        ...
        if(genoarr_widx >= genoarr_widx_trail) {
            ...
            ww = ProperSubwordLoad(&(onebit_main_iter[genoarr_widx_trail * sizeof(Halfword)]),
                                   1 + (((raw_sample_ct - 1) % kBitsPerWordD2) / CHAR_BIT));
        } else {
            Halfword hw;
...
PglErr PgrGetDifflistOrGenovec(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct,
                               ...
                               uint32_t *__restrict difflist_common_geno_ptr, uintptr_t *__restrict main_raregeno,
                               uint32_t *__restrict difflist_sample_ids, uint32_t *__restrict difflist_len_ptr) {
    if(!sample_ct) {
        *difflist_common_geno_ptr = UINT32_MAX;
        return kPglRetSuccess;
...
void CountAux1bDense(const unsigned char *patch_10_fvals, uint32_t allele_ct, uint32_t allele_idx_m1, uint32_t raw_10_ct, uint32_t rare10_ct,
...
static inline void SuppressHets00(const uintptr_t *allele_countvec, uintptr_t *subsetted_all_hets, uint32_t sample_ct,
                                  uintptr_t *subsetted_suppressed_hets) {
...
PglErr Get1Multiallelic(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                        ...
                        uintptr_t **subsetted_suppressed_hetp) {
...
PglErr IMPLPgrGetInv1DifflistOrGenovec(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                                       uint32_t sample_ct, uint32_t max_simple_difflist_len, uint32_t vidx, uint32_t allele_idx, PgenReaderMain *pgrp,
                                       uintptr_t *__restrict allele_invcountvec, uint32_t *__restrict difflist_common_geno_ptr,
                                       uintptr_t *__restrict main_raregeno, uint32_t *__restrict difflist_sample_ids,
    ...
    const uint32_t multiallelic_hc_present = VrtypeMultiallelicHc(vrtype);
    if((!allele_idx) || ((allele_idx == 1) && (!multiallelic_hc_present))) {
        PglErr reterr = ReadDifflistOrGenovecSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, max_simple_difflist_len,
                                                          vidx, pgrp, nullptr, nullptr, allele_invcountvec, difflist_common_geno_ptr, main_raregeno,
...
PglErr ParseAux2Subset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, const uintptr_t *__restrict all_hets,
                       const uintptr_t *__restrict subsetted_suppressed_het, uint32_t raw_sample_ct, uint32_t sample_ct,
                       const unsigned char **fread_pp, uintptr_t *__restrict phasepresent, uintptr_t *__restrict phaseinfo,
                       uint32_t *__restrict phasepresent_ct_ptr, uintptr_t *__restrict workspace_subset) {
    const uint32_t raw_sample_ctl = BitCtToWordCt(raw_sample_ct);
...
PglErr ReadGenovecHphaseSubsetUnsafe(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
    ...
    reterr = ParseAux2Subset(fread_end, subsetting_required ? sample_include : nullptr, all_hets, subsetted_suppressed_het, raw_sample_ct, sample_ct,
...
PglErr Get1MP(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
    ...
    PglErr reterr = Get1Multiallelic(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, &fread_ptr, &fread_end,
                                     all_hets, allele_countvec, &subsetted_suppressed_het);
    if(unlikely(reterr)) {
        return reterr;
    }
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    reterr = ParseAux2Subset(fread_end, (sample_ct != raw_sample_ct) ? sample_include : nullptr, all_hets, subsetted_suppressed_het, raw_sample_ct,
                             sample_ct, &fread_ptr, phasepresent, phaseinfo, phasepresent_ct_ptr, pgrp->workspace_subset);
...
PglErr PgrGet2P(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, uint32_t allele_idx0,
    ...
    reterr = ParseAux2Subset(fread_end, sample_include, all_hets, subsetted_suppressed_het, raw_sample_ct, sample_ct, &fread_ptr, phasepresent,
                             phaseinfo, phasepresent_ct_ptr, pgrp->workspace_subset);
    if(unlikely(reterr)) {
        return reterr;
    }
...
PglErr PgrGetRaw(uint32_t vidx, PgenGlobalFlags read_gflags, PgenReader *pgr_ptr, uintptr_t **loadbuf_iter_ptr, unsigned char *loaded_vrtype_ptr) {
    ...
    *loadbuf_iter_ptr = loadbuf_iter;
    reterr = ParseDosage16(fread_ptr, fread_end, nullptr, raw_sample_ct, vidx, allele_ct, pgrp, nullptr, dphase_present, dphase_delta, nullptr,
                           dosage_present, dosage_main);
    return reterr;
}
