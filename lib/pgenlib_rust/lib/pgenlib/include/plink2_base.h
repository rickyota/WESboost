...
#define container_of(ptr, type, member) (R_CAST(type *, R_CAST(char *, ptr) - offsetof(type, member)))
...
#define const_container_of(ptr, type, member) (R_CAST(const type *, R_CAST(const char *, ptr) - offsetof(type, member)))
...
HEADER_INLINE double swtod(intptr_t lii) { return S_CAST(double, lii); }
...
HEADER_INLINE uint32_t ctou32(char cc) { return S_CAST(unsigned char, cc); }

HEADER_INLINE uintptr_t ctow(char cc) { return S_CAST(unsigned char, cc); }

HEADER_INLINE uint64_t ctou64(char cc) { return S_CAST(unsigned char, cc); }

HEADER_INLINE unsigned char *CToUc(char *pp) { return R_CAST(unsigned char *, pp); }
...
typedef enum
    ...
    operator ec() const { return value_; }
    ...
    explicit operator bool() const { return (static_cast<uint32_t>(value_) != 0); }
...
struct IntErr {
    ...
    explicit operator int32_t() const { return value_; }
...
HEADER_INLINE uint32_t ctzu32(uint32_t uii) { return __builtin_ctz(uii); }

// this should always compile down to bsr.
HEADER_INLINE uint32_t bsru32(uint32_t uii) { return 31 - __builtin_clz(uii); }

#ifdef _WIN64
HEADER_INLINE uint32_t ctzw(unsigned long long ullii) { return __builtin_ctzll(ullii); }
...
#else
HEADER_INLINE uint32_t ctzw(unsigned long ulii) { return __builtin_ctzl(ulii); }

HEADER_INLINE uint32_t bsrw(unsigned long ulii) { return (8 * sizeof(intptr_t) - 1) - __builtin_clzl(ulii); }
#ifndef __LP64__
...
HEADER_INLINE VecW VecToW(__m256i vv) { return R_CAST(VecW, vv); }

HEADER_INLINE VecU32 VecToU32(__m256i vv) { return R_CAST(VecU32, vv); }

HEADER_INLINE VecI32 VecToI32(__m256i vv) { return R_CAST(VecI32, vv); }

HEADER_INLINE VecU16 VecToU16(__m256i vv) { return R_CAST(VecU16, vv); }

HEADER_INLINE VecI16 VecToI16(__m256i vv) { return R_CAST(VecI16, vv); }

HEADER_INLINE VecUc VecToUc(__m256i vv) { return R_CAST(VecUc, vv); }

HEADER_INLINE VecI8 VecToI8(__m256i vv) { return R_CAST(VecI8, vv); }

HEADER_INLINE __m256i WToVec(VecW vv) { return R_CAST(__m256i, vv); }

HEADER_INLINE __m256i U32ToVec(VecU32 vv) { return R_CAST(__m256i, vv); }

HEADER_INLINE __m256i I32ToVec(VecI32 vv) { return R_CAST(__m256i, vv); }

HEADER_INLINE __m256i U16ToVec(VecU16 vv) { return R_CAST(__m256i, vv); }

HEADER_INLINE __m256i I16ToVec(VecI16 vv) { return R_CAST(__m256i, vv); }

HEADER_INLINE __m256i UcToVec(VecUc vv) { return R_CAST(__m256i, vv); }

HEADER_INLINE __m256i I8ToVec(VecI8 vv) { return R_CAST(__m256i, vv); }
...
HEADER_INLINE VecW vecw_setzero() { return VecToW(_mm256_setzero_si256()); }

HEADER_INLINE VecU32 vecu32_setzero() { return VecToU32(_mm256_setzero_si256()); }

HEADER_INLINE VecU16 vecu16_setzero() { return VecToU16(_mm256_setzero_si256()); }

HEADER_INLINE VecI16 veci16_setzero() { return VecToI16(_mm256_setzero_si256()); }

HEADER_INLINE VecUc vecuc_setzero() { return VecToUc(_mm256_setzero_si256()); }

HEADER_INLINE VecI8 veci8_setzero() { return VecToI8(_mm256_setzero_si256()); }

// "vv >> ct" doesn't work, and Scientific Linux gcc 4.4 might not optimize
// VCONST_W shift properly (todo: test this)
HEADER_INLINE VecW vecw_srli(VecW vv, uint32_t ct) { return VecToW(_mm256_srli_epi64(WToVec(vv), ct)); }

HEADER_INLINE VecW vecw_slli(VecW vv, uint32_t ct) { return VecToW(_mm256_slli_epi64(WToVec(vv), ct)); }

HEADER_INLINE VecU32 vecu32_srli(VecU32 vv, uint32_t ct) { return VecToU32(_mm256_srli_epi32(U32ToVec(vv), ct)); }

HEADER_INLINE VecU32 vecu32_slli(VecU32 vv, uint32_t ct) { return VecToU32(_mm256_slli_epi32(U32ToVec(vv), ct)); }

HEADER_INLINE VecU16 vecu16_srli(VecU16 vv, uint32_t ct) { return VecToU16(_mm256_srli_epi16(U16ToVec(vv), ct)); }

HEADER_INLINE VecU16 vecu16_slli(VecU16 vv, uint32_t ct) { return VecToU16(_mm256_slli_epi16(U16ToVec(vv), ct)); }

// Compiler still doesn't seem to be smart enough to use andnot properly.
HEADER_INLINE VecW vecw_and_notfirst(VecW excl, VecW main) { return VecToW(_mm256_andnot_si256(WToVec(excl), WToVec(main))); }

HEADER_INLINE VecU32 vecu32_and_notfirst(VecU32 excl, VecU32 main) { return VecToU32(_mm256_andnot_si256(U32ToVec(excl), U32ToVec(main))); }

HEADER_INLINE VecI32 veci32_and_notfirst(VecI32 excl, VecI32 main) { return VecToI32(_mm256_andnot_si256(I32ToVec(excl), I32ToVec(main))); }

HEADER_INLINE VecU16 vecu16_and_notfirst(VecU16 excl, VecU16 main) { return VecToU16(_mm256_andnot_si256(U16ToVec(excl), U16ToVec(main))); }

HEADER_INLINE VecI16 veci16_and_notfirst(VecI16 excl, VecI16 main) { return VecToI16(_mm256_andnot_si256(I16ToVec(excl), I16ToVec(main))); }

HEADER_INLINE VecUc vecuc_and_notfirst(VecUc excl, VecUc main) { return VecToUc(_mm256_andnot_si256(UcToVec(excl), UcToVec(main))); }

HEADER_INLINE VecI8 veci8_and_notfirst(VecI8 excl, VecI8 main) { return VecToI8(_mm256_andnot_si256(I8ToVec(excl), I8ToVec(main))); }

HEADER_INLINE VecW vecw_set1(uintptr_t ulii) { return VecToW(_mm256_set1_epi64x(ulii)); }

HEADER_INLINE VecU32 vecu32_set1(uint32_t uii) { return VecToU32(_mm256_set1_epi32(uii)); }

HEADER_INLINE VecI32 veci32_set1(int32_t ii) { return VecToI32(_mm256_set1_epi32(ii)); }

HEADER_INLINE VecU16 vecu16_set1(unsigned short usi) { return VecToU16(_mm256_set1_epi16(usi)); }

HEADER_INLINE VecI16 veci16_set1(short si) { return VecToI16(_mm256_set1_epi16(si)); }

HEADER_INLINE VecUc vecuc_set1_epi16(unsigned short usi) { return VecToUc(_mm256_set1_epi16(usi)); }

HEADER_INLINE VecUc vecuc_set1(unsigned char ucc) { return VecToUc(_mm256_set1_epi8(ucc)); }

HEADER_INLINE VecI8 veci8_set1(char cc) { return VecToI8(_mm256_set1_epi8(cc)); }

HEADER_INLINE uint32_t vecw_movemask(VecW vv) { return _mm256_movemask_epi8(WToVec(vv)); }

HEADER_INLINE uint32_t vecu32_movemask(VecU32 vv) { return _mm256_movemask_epi8(U32ToVec(vv)); }

HEADER_INLINE uint32_t veci32_movemask(VecI32 vv) { return _mm256_movemask_epi8(I32ToVec(vv)); }

HEADER_INLINE uint32_t vecu16_movemask(VecU16 vv) { return _mm256_movemask_epi8(U16ToVec(vv)); }

HEADER_INLINE uint32_t veci16_movemask(VecI16 vv) { return _mm256_movemask_epi8(I16ToVec(vv)); }

HEADER_INLINE uint32_t veci8_movemask(VecI8 vv) { return _mm256_movemask_epi8(I8ToVec(vv)); }

HEADER_INLINE uint32_t vecuc_movemask(VecUc vv) { return _mm256_movemask_epi8(UcToVec(vv)); }

// Repeats elements in second lane in AVX2 case.
HEADER_INLINE VecW vecw_setr8(char e15, char e14, char e13, char e12, char e11, char e10, char e9, char e8, char e7, char e6, char e5, char e4,
                              char e3, char e2, char e1, char e0) {
    return VecToW(_mm256_setr_epi8(e15, e14, e13, e12, e11, e10, e9, e8, e7, e6, e5, e4, e3, e2, e1, e0, e15, e14, e13, e12, e11, e10, e9, e8, e7, e6,
                                   e5, e4, e3, e2, e1, e0));
}

HEADER_INLINE VecU16 vecu16_setr8(char e15, char e14, char e13, char e12, char e11, char e10, char e9, char e8, char e7, char e6, char e5, char e4,
                                  char e3, char e2, char e1, char e0) {
    return VecToU16(_mm256_setr_epi8(e15, e14, e13, e12, e11, e10, e9, e8, e7, e6, e5, e4, e3, e2, e1, e0, e15, e14, e13, e12, e11, e10, e9, e8, e7,
                                     e6, e5, e4, e3, e2, e1, e0));
}

HEADER_INLINE VecUc vecuc_setr8(char e15, char e14, char e13, char e12, char e11, char e10, char e9, char e8, char e7, char e6, char e5, char e4,
                                char e3, char e2, char e1, char e0) {
    return VecToUc(_mm256_setr_epi8(e15, e14, e13, e12, e11, e10, e9, e8, e7, e6, e5, e4, e3, e2, e1, e0, e15, e14, e13, e12, e11, e10, e9, e8, e7,
                                    e6, e5, e4, e3, e2, e1, e0));
}

// Discards last 16 arguments in SSE2/SSE4.2 case.
HEADER_INLINE VecW vecw_setr8x(char e31, char e30, char e29, char e28, char e27, char e26, char e25, char e24, char e23, char e22, char e21, char e20,
                               char e19, char e18, char e17, char e16, char e15, char e14, char e13, char e12, char e11, char e10, char e9, char e8,
                               char e7, char e6, char e5, char e4, char e3, char e2, char e1, char e0) {
    return VecToW(_mm256_setr_epi8(e31, e30, e29, e28, e27, e26, e25, e24, e23, e22, e21, e20, e19, e18, e17, e16, e15, e14, e13, e12, e11, e10, e9,
                                   e8, e7, e6, e5, e4, e3, e2, e1, e0));
}

HEADER_INLINE VecUc vecuc_setr8x(char e31, char e30, char e29, char e28, char e27, char e26, char e25, char e24, char e23, char e22, char e21,
                                 char e20, char e19, char e18, char e17, char e16, char e15, char e14, char e13, char e12, char e11, char e10,
                                 char e9, char e8, char e7, char e6, char e5, char e4, char e3, char e2, char e1, char e0) {
    return VecToUc(_mm256_setr_epi8(e31, e30, e29, e28, e27, e26, e25, e24, e23, e22, e21, e20, e19, e18, e17, e16, e15, e14, e13, e12, e11, e10, e9,
                                    e8, e7, e6, e5, e4, e3, e2, e1, e0));
}
...
HEADER_INLINE VecW vecw_unpacklo8(VecW evens, VecW odds) { return VecToW(_mm256_unpacklo_epi8(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi8(VecW evens, VecW odds) { return VecToW(_mm256_unpackhi_epi8(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecI8 veci8_unpacklo8(VecI8 evens, VecI8 odds) { return VecToI8(_mm256_unpacklo_epi8(I8ToVec(evens), I8ToVec(odds))); }

HEADER_INLINE VecI8 veci8_unpackhi8(VecI8 evens, VecI8 odds) { return VecToI8(_mm256_unpackhi_epi8(I8ToVec(evens), I8ToVec(odds))); }

HEADER_INLINE VecUc vecuc_unpacklo8(VecUc evens, VecUc odds) { return VecToUc(_mm256_unpacklo_epi8(UcToVec(evens), UcToVec(odds))); }

HEADER_INLINE VecUc vecuc_unpackhi8(VecUc evens, VecUc odds) { return VecToUc(_mm256_unpackhi_epi8(UcToVec(evens), UcToVec(odds))); }

HEADER_INLINE VecW vecw_unpacklo16(VecW evens, VecW odds) { return VecToW(_mm256_unpacklo_epi16(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi16(VecW evens, VecW odds) { return VecToW(_mm256_unpackhi_epi16(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpacklo32(VecW evens, VecW odds) { return VecToW(_mm256_unpacklo_epi32(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi32(VecW evens, VecW odds) { return VecToW(_mm256_unpackhi_epi32(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_permute0xd8_if_avx2(VecW vv) { return VecToW(_mm256_permute4x64_epi64(WToVec(vv), 0xd8)); }

HEADER_INLINE VecI8 veci8_permute0xd8_if_avx2(VecI8 vv) { return VecToI8(_mm256_permute4x64_epi64(I8ToVec(vv), 0xd8)); }

HEADER_INLINE VecUc vecuc_permute0xd8_if_avx2(VecUc vv) { return VecToUc(_mm256_permute4x64_epi64(UcToVec(vv), 0xd8)); }
...
HEADER_INLINE VecW vecw_shuffle8(VecW table, VecW indexes) { return VecToW(_mm256_shuffle_epi8(WToVec(table), WToVec(indexes))); }

HEADER_INLINE VecU16 vecu16_shuffle8(VecU16 table, VecU16 indexes) { return VecToU16(_mm256_shuffle_epi8(U16ToVec(table), U16ToVec(indexes))); }

HEADER_INLINE VecUc vecuc_shuffle8(VecUc table, VecUc indexes) { return VecToUc(_mm256_shuffle_epi8(UcToVec(table), UcToVec(indexes))); }

HEADER_INLINE uintptr_t vecw_extract64_0(VecW vv) { return _mm256_extract_epi64(WToVec(vv), 0); }

HEADER_INLINE uintptr_t vecw_extract64_1(VecW vv) { return _mm256_extract_epi64(WToVec(vv), 1); }

// *** AVX2-only section ***
HEADER_INLINE uintptr_t vecw_extract64_2(VecW vv) { return _mm256_extract_epi64(WToVec(vv), 2); }

HEADER_INLINE uintptr_t vecw_extract64_3(VecW vv) { return _mm256_extract_epi64(WToVec(vv), 3); }
...
HEADER_INLINE VecW vecw_load(const void *mem_addr) { return VecToW(_mm256_load_si256(S_CAST(const __m256i *, mem_addr))); }
...
HEADER_INLINE VecW vecw_loadu(const void *mem_addr) { return VecToW(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE VecU32 vecu32_loadu(const void *mem_addr) { return VecToU32(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE VecI32 veci32_loadu(const void *mem_addr) { return VecToI32(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE VecU16 vecu16_loadu(const void *mem_addr) { return VecToU16(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE VecI16 veci16_loadu(const void *mem_addr) { return VecToI16(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE VecUc vecuc_loadu(const void *mem_addr) { return VecToUc(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE VecI8 veci8_loadu(const void *mem_addr) { return VecToI8(_mm256_loadu_si256(S_CAST(const __m256i *, mem_addr))); }

HEADER_INLINE void vec_storeu(void *mem_addr, __m256i vv) { _mm256_storeu_si256(S_CAST(__m256i *, mem_addr), vv); }

HEADER_INLINE VecI32 veci32_max(VecI32 v1, VecI32 v2) { return VecToI32(_mm256_max_epi32(I32ToVec(v1), I32ToVec(v2))); }

HEADER_INLINE VecI16 veci16_max(VecI16 v1, VecI16 v2) { return VecToI16(_mm256_max_epi16(I16ToVec(v1), I16ToVec(v2))); }

HEADER_INLINE VecW vecw_sad(VecW v1, VecW v2) { return VecToW(_mm256_sad_epu8(WToVec(v1), WToVec(v2))); }

HEADER_INLINE VecUc vecuc_adds(VecUc v1, VecUc v2) { return VecToUc(_mm256_adds_epu8(UcToVec(v1), UcToVec(v2))); }

HEADER_INLINE VecU16 vecu16_min8(VecU16 v1, VecU16 v2) { return VecToU16(_mm256_min_epu8(U16ToVec(v1), U16ToVec(v2))); }

HEADER_INLINE VecUc vecuc_min(VecUc v1, VecUc v2) { return VecToUc(_mm256_min_epu8(UcToVec(v1), UcToVec(v2))); }

HEADER_INLINE VecW vecw_blendv(VecW aa, VecW bb, VecW mask) { return VecToW(_mm256_blendv_epi8(WToVec(aa), WToVec(bb), WToVec(mask))); }
...
HEADER_INLINE VecUc vecuc_blendv(VecUc aa, VecUc bb, VecUc mask) { return VecToUc(_mm256_blendv_epi8(UcToVec(aa), UcToVec(bb), UcToVec(mask))); }
...
HEADER_INLINE VecW VecToW(__m128i vv) { return R_CAST(VecW, vv); }

HEADER_INLINE VecU32 VecToU32(__m128i vv) { return R_CAST(VecU32, vv); }

HEADER_INLINE VecI32 VecToI32(__m128i vv) { return R_CAST(VecI32, vv); }

HEADER_INLINE VecU16 VecToU16(__m128i vv) { return R_CAST(VecU16, vv); }

HEADER_INLINE VecI16 VecToI16(__m128i vv) { return R_CAST(VecI16, vv); }

HEADER_INLINE VecUc VecToUc(__m128i vv) { return R_CAST(VecUc, vv); }

HEADER_INLINE VecI8 VecToI8(__m128i vv) { return R_CAST(VecI8, vv); }

HEADER_INLINE __m128i WToVec(VecW vv) { return R_CAST(__m128i, vv); }

HEADER_INLINE __m128i U32ToVec(VecU32 vv) { return R_CAST(__m128i, vv); }

HEADER_INLINE __m128i I32ToVec(VecI32 vv) { return R_CAST(__m128i, vv); }

HEADER_INLINE __m128i U16ToVec(VecU16 vv) { return R_CAST(__m128i, vv); }

HEADER_INLINE __m128i I16ToVec(VecI16 vv) { return R_CAST(__m128i, vv); }

HEADER_INLINE __m128i UcToVec(VecUc vv) { return R_CAST(__m128i, vv); }

HEADER_INLINE __m128i I8ToVec(VecI8 vv) { return R_CAST(__m128i, vv); }
...
HEADER_INLINE VecW vecw_setzero() { return VecToW(_mm_setzero_si128()); }

HEADER_INLINE VecU32 vecu32_setzero() { return VecToU32(_mm_setzero_si128()); }

HEADER_INLINE VecU16 vecu16_setzero() { return VecToU16(_mm_setzero_si128()); }

HEADER_INLINE VecI16 veci16_setzero() { return VecToI16(_mm_setzero_si128()); }

HEADER_INLINE VecUc vecuc_setzero() { return VecToUc(_mm_setzero_si128()); }

HEADER_INLINE VecI8 veci8_setzero() { return VecToI8(_mm_setzero_si128()); }
...
HEADER_INLINE VecW vecw_and_notfirst(VecW excl, VecW main) { return VecToW(_mm_andnot_si128(WToVec(excl), WToVec(main))); }

HEADER_INLINE VecU32 vecu32_and_notfirst(VecU32 excl, VecU32 main) { return VecToU32(_mm_andnot_si128(U32ToVec(excl), U32ToVec(main))); }

HEADER_INLINE VecI32 veci32_and_notfirst(VecI32 excl, VecI32 main) { return VecToI32(_mm_andnot_si128(I32ToVec(excl), I32ToVec(main))); }

HEADER_INLINE VecU16 vecu16_and_notfirst(VecU16 excl, VecU16 main) { return VecToU16(_mm_andnot_si128(U16ToVec(excl), U16ToVec(main))); }

HEADER_INLINE VecI16 veci16_and_notfirst(VecI16 excl, VecI16 main) { return VecToI16(_mm_andnot_si128(I16ToVec(excl), I16ToVec(main))); }

HEADER_INLINE VecUc vecuc_and_notfirst(VecUc excl, VecUc main) { return VecToUc(_mm_andnot_si128(UcToVec(excl), UcToVec(main))); }

HEADER_INLINE VecI8 veci8_and_notfirst(VecI8 excl, VecI8 main) { return VecToI8(_mm_andnot_si128(I8ToVec(excl), I8ToVec(main))); }

HEADER_INLINE VecW vecw_set1(uintptr_t ulii) { return VecToW(_mm_set1_epi64x(ulii)); }

HEADER_INLINE VecU32 vecu32_set1(uint32_t uii) { return VecToU32(_mm_set1_epi32(uii)); }

HEADER_INLINE VecI32 veci32_set1(int32_t ii) { return VecToI32(_mm_set1_epi32(ii)); }

HEADER_INLINE VecU16 vecu16_set1(unsigned short usi) { return VecToU16(_mm_set1_epi16(usi)); }

HEADER_INLINE VecI16 veci16_set1(short si) { return VecToI16(_mm_set1_epi16(si)); }

HEADER_INLINE VecUc vecuc_set1_epi16(unsigned short usi) { return VecToUc(_mm_set1_epi16(usi)); }

HEADER_INLINE VecUc vecuc_set1(unsigned char ucc) { return VecToUc(_mm_set1_epi8(ucc)); }

HEADER_INLINE VecI8 veci8_set1(char cc) { return VecToI8(_mm_set1_epi8(cc)); }

HEADER_INLINE uint32_t vecw_movemask(VecW vv) { return _mm_movemask_epi8(WToVec(vv)); }

HEADER_INLINE uint32_t vecu32_movemask(VecU32 vv) { return _mm_movemask_epi8(U32ToVec(vv)); }

HEADER_INLINE uint32_t veci32_movemask(VecI32 vv) { return _mm_movemask_epi8(I32ToVec(vv)); }

HEADER_INLINE uint32_t vecu16_movemask(VecU16 vv) { return _mm_movemask_epi8(U16ToVec(vv)); }

HEADER_INLINE uint32_t veci16_movemask(VecI16 vv) { return _mm_movemask_epi8(I16ToVec(vv)); }

HEADER_INLINE uint32_t veci8_movemask(VecI8 vv) { return _mm_movemask_epi8(I8ToVec(vv)); }

HEADER_INLINE uint32_t vecuc_movemask(VecUc vv) { return _mm_movemask_epi8(UcToVec(vv)); }
...
HEADER_INLINE VecW vecw_load(const void *mem_addr) { return VecToW(_mm_load_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecW vecw_loadu(const void *mem_addr) { return VecToW(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecU32 vecu32_loadu(const void *mem_addr) { return VecToU32(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecI32 veci32_loadu(const void *mem_addr) { return VecToI32(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecU16 vecu16_loadu(const void *mem_addr) { return VecToU16(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecI16 veci16_loadu(const void *mem_addr) { return VecToI16(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecUc vecuc_loadu(const void *mem_addr) { return VecToUc(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE VecI8 veci8_loadu(const void *mem_addr) { return VecToI8(_mm_loadu_si128(S_CAST(const __m128i *, mem_addr))); }

HEADER_INLINE void vec_storeu(void *mem_addr, __m128i vv) { _mm_storeu_si128(S_CAST(__m128i *, mem_addr), vv); }
...
// Discards last 16 arguments in SSE2/SSE4.2 case.
HEADER_INLINE VecW vecw_setr8x(char e31, char e30, char e29, char e28, char e27, char e26, char e25, char e24, char e23, char e22, char e21, char e20,
                               char e19, char e18, char e17, char e16, __maybe_unused char e15, __maybe_unused char e14, __maybe_unused char e13,
                               __maybe_unused char e12, __maybe_unused char e11, __maybe_unused char e10, __maybe_unused char e9,
                               __maybe_unused char e8, __maybe_unused char e7, __maybe_unused char e6, __maybe_unused char e5, __maybe_unused char e4,
                               __maybe_unused char e3, __maybe_unused char e2, __maybe_unused char e1, __maybe_unused char e0) {
    return VecToW(_mm_setr_epi8(e31, e30, e29, e28, e27, e26, e25, e24, e23, e22, e21, e20, e19, e18, e17, e16));
}

HEADER_INLINE VecUc vecuc_setr8x(char e31, char e30, char e29, char e28, char e27, char e26, char e25, char e24, char e23, char e22, char e21,
                                 char e20, char e19, char e18, char e17, char e16, __maybe_unused char e15, __maybe_unused char e14,
                                 __maybe_unused char e13, __maybe_unused char e12, __maybe_unused char e11, __maybe_unused char e10,
                                 __maybe_unused char e9, __maybe_unused char e8, __maybe_unused char e7, __maybe_unused char e6,
                                 __maybe_unused char e5, __maybe_unused char e4, __maybe_unused char e3, __maybe_unused char e2,
...
HEADER_INLINE VecW vecw_setr32(uint32_t e3, uint32_t e2, uint32_t e1, uint32_t e0) { return VecToW(_mm_setr_epi32(e3, e2, e1, e0)); }

HEADER_INLINE VecW vecw_unpacklo8(VecW evens, VecW odds) { return VecToW(_mm_unpacklo_epi8(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi8(VecW evens, VecW odds) { return VecToW(_mm_unpackhi_epi8(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecI8 veci8_unpacklo8(VecI8 evens, VecI8 odds) { return VecToI8(_mm_unpacklo_epi8(I8ToVec(evens), I8ToVec(odds))); }

HEADER_INLINE VecI8 veci8_unpackhi8(VecI8 evens, VecI8 odds) { return VecToI8(_mm_unpackhi_epi8(I8ToVec(evens), I8ToVec(odds))); }

HEADER_INLINE VecUc vecuc_unpacklo8(VecUc evens, VecUc odds) { return VecToUc(_mm_unpacklo_epi8(UcToVec(evens), UcToVec(odds))); }

HEADER_INLINE VecUc vecuc_unpackhi8(VecUc evens, VecUc odds) { return VecToUc(_mm_unpackhi_epi8(UcToVec(evens), UcToVec(odds))); }

HEADER_INLINE VecW vecw_unpacklo16(VecW evens, VecW odds) { return VecToW(_mm_unpacklo_epi16(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi16(VecW evens, VecW odds) { return VecToW(_mm_unpackhi_epi16(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpacklo32(VecW evens, VecW odds) { return VecToW(_mm_unpacklo_epi32(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi32(VecW evens, VecW odds) { return VecToW(_mm_unpackhi_epi32(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpacklo64(VecW evens, VecW odds) { return VecToW(_mm_unpacklo_epi64(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_unpackhi64(VecW evens, VecW odds) { return VecToW(_mm_unpackhi_epi64(WToVec(evens), WToVec(odds))); }

HEADER_INLINE VecW vecw_permute0xd8_if_avx2(VecW vv) { return vv; }

HEADER_INLINE VecI8 veci8_permute0xd8_if_avx2(VecI8 vv) { return vv; }

HEADER_INLINE VecUc vecuc_permute0xd8_if_avx2(VecUc vv) { return vv; }

HEADER_INLINE VecW vecw_gather_even(VecW src_lo, VecW src_hi, VecW m8) { return VecToW(_mm_packus_epi16(WToVec(src_lo & m8), WToVec(src_hi & m8))); }
...
#endif
HEADER_INLINE VecW vecw_shuffle8(VecW table, VecW indexes) { return VecToW(_mm_shuffle_epi8(WToVec(table), WToVec(indexes))); }

HEADER_INLINE VecU16 vecu16_shuffle8(VecU16 table, VecU16 indexes) { return VecToU16(_mm_shuffle_epi8(U16ToVec(table), U16ToVec(indexes))); }

HEADER_INLINE VecUc vecuc_shuffle8(VecUc table, VecUc indexes) { return VecToUc(_mm_shuffle_epi8(UcToVec(table), UcToVec(indexes))); }
#endif
#ifdef USE_SSE42
HEADER_INLINE VecI32 veci32_max(VecI32 v1, VecI32 v2) { return VecToI32(_mm_max_epi32(I32ToVec(v1), I32ToVec(v2))); }

HEADER_INLINE uintptr_t vecw_extract64_0(VecW vv) { return _mm_extract_epi64(WToVec(vv), 0); }

HEADER_INLINE uintptr_t vecw_extract64_1(VecW vv) { return _mm_extract_epi64(WToVec(vv), 1); }

HEADER_INLINE VecW vecw_blendv(VecW aa, VecW bb, VecW mask) { return VecToW(_mm_blendv_epi8(WToVec(aa), WToVec(bb), WToVec(mask))); }
...
HEADER_INLINE VecUc vecuc_blendv(VecUc aa, VecUc bb, VecUc mask) { return VecToUc(_mm_blendv_epi8(UcToVec(aa), UcToVec(bb), UcToVec(mask))); }
#else // USE_SSE2, !USE_SSE42
HEADER_INLINE uintptr_t vecw_extract64_0(VecW vv) { return R_CAST(uintptr_t, _mm_movepi64_pi64(WToVec(vv))); }
...
// bit.
HEADER_INLINE VecW vecw_blendv(VecW aa, VecW bb, VecW mask) { return vecw_and_notfirst(mask, aa) | (mask & bb); }

HEADER_INLINE VecU32 vecu32_blendv(VecU32 aa, VecU32 bb, VecU32 mask) { return vecu32_and_notfirst(mask, aa) | (mask & bb); }

HEADER_INLINE VecU16 vecu16_blendv(VecU16 aa, VecU16 bb, VecU16 mask) { return vecu16_and_notfirst(mask, aa) | (mask & bb); }

HEADER_INLINE VecUc vecuc_blendv(VecUc aa, VecUc bb, VecUc mask) { return vecuc_and_notfirst(mask, aa) | (mask & bb); }
#endif

HEADER_INLINE VecI16 veci16_max(VecI16 v1, VecI16 v2) { return VecToI16(_mm_max_epi16(I16ToVec(v1), I16ToVec(v2))); }

HEADER_INLINE VecW vecw_sad(VecW v1, VecW v2) { return VecToW(_mm_sad_epu8(WToVec(v1), WToVec(v2))); }

HEADER_INLINE VecUc vecuc_adds(VecUc v1, VecUc v2) { return VecToUc(_mm_adds_epu8(UcToVec(v1), UcToVec(v2))); }

HEADER_INLINE VecU16 vecu16_min8(VecU16 v1, VecU16 v2) { return VecToU16(_mm_min_epu8(U16ToVec(v1), U16ToVec(v2))); }

HEADER_INLINE VecUc vecuc_min(VecUc v1, VecUc v2) { return VecToUc(_mm_min_epu8(UcToVec(v1), UcToVec(v2))); }
#endif // USE_SSE2, !USE_AVX2

HEADER_INLINE void vecw_storeu(void *mem_addr, VecW vv) { vec_storeu(mem_addr, WToVec(vv)); }

HEADER_INLINE void vecu32_storeu(void *mem_addr, VecU32 vv) { vec_storeu(mem_addr, U32ToVec(vv)); }

HEADER_INLINE void veci32_storeu(void *mem_addr, VecI32 vv) { vec_storeu(mem_addr, I32ToVec(vv)); }

HEADER_INLINE void vecu16_storeu(void *mem_addr, VecU16 vv) { vec_storeu(mem_addr, U16ToVec(vv)); }

HEADER_INLINE void veci16_storeu(void *mem_addr, VecI16 vv) { vec_storeu(mem_addr, I16ToVec(vv)); }

HEADER_INLINE void vecuc_storeu(void *mem_addr, VecUc vv) { vec_storeu(mem_addr, UcToVec(vv)); }

HEADER_INLINE VecW vecw_bytesum(VecW src, VecW m0) { return vecw_sad(src, m0); }
...
HEADER_INLINE VecF VecToF(__m256 xxv) { return R_CAST(VecF, xxv); }

HEADER_INLINE VecD VecToD(__m256d xxv) { return R_CAST(VecD, xxv); }

HEADER_INLINE __m256 FToVec(VecF xxv) { return R_CAST(__m256, xxv); }

HEADER_INLINE __m256d DToVec(VecD xxv) { return R_CAST(__m256d, xxv); }

HEADER_INLINE VecF vecf_setzero() { return VecToF(_mm256_setzero_ps()); }

HEADER_INLINE VecD vecd_setzero() { return VecToD(_mm256_setzero_pd()); }
...
HEADER_INLINE VecF VecToF(__m128 xxv) { return R_CAST(VecF, xxv); }

HEADER_INLINE VecD VecToD(__m128d xxv) { return R_CAST(VecD, xxv); }

HEADER_INLINE __m128 FToVec(VecF xxv) { return R_CAST(__m128, xxv); }

HEADER_INLINE __m128d DToVec(VecD xxv) { return R_CAST(__m128d, xxv); }

HEADER_INLINE VecF vecf_setzero() { return VecToF(_mm_setzero_ps()); }

HEADER_INLINE VecD vecd_setzero() { return VecToD(_mm_setzero_pd()); }

#endif // !FVEC_32

HEADER_INLINE VecUc VecWToUc(VecW vv) { return R_CAST(VecUc, vv); }

HEADER_INLINE VecW VecU16ToW(VecU16 vv) { return R_CAST(VecW, vv); }

HEADER_INLINE VecW VecUcToW(VecUc vv) { return R_CAST(VecW, vv); }
...
HEADER_INLINE VecW vecw_setzero() { return k0LU; }

HEADER_INLINE VecW vecw_srli(VecW vv, uint32_t ct) { return vv >> ct; }

HEADER_INLINE VecW vecw_slli(VecW vv, uint32_t ct) { return vv << ct; }

HEADER_INLINE VecW vecw_set1(uintptr_t ulii) { return ulii; }

HEADER_INLINE VecW vecw_loadu(const void *mem_addr) { return *S_CAST(const VecW *, mem_addr); }
...
HEADER_INLINE VecW vecw_and_notfirst(VecW excl, VecW main) { return (~excl) & main; }

HEADER_INLINE VecU32 vecu32_and_notfirst(VecU32 excl, VecU32 main) { return (~excl) & main; }
#endif // !USE_SSE2

HEADER_INLINE uint32_t *I32ToU32(int32_t *pp) { return R_CAST(uint32_t *, pp); }
...
#define STD_ARRAY_COPY(src, nn, dst)                                                                                                                 \
    static_assert(sizeof(dst) == sizeof((dst)[0]) * nn, "Invalid STD_ARRAY_COPY() invocation.");                                                     \
    (dst) = (src)
...
template <class T, std::size_t N> void STD_ARRAY_FILL0(std::array<T, N> &arr) { arr.fill(0); }
...
#define STD_ARRAY_REF_FILL0(ct, aref)                                                                                                                \
    static_assert(ct * sizeof(aref[0]) == sizeof(aref), "invalid STD_ARRAY_REF_FILL0() invocation");                                                 \
    aref.fill(0)
...
#ifdef USE_AVX2
HEADER_INLINE uintptr_t UnpackHalfwordToWord(uintptr_t hw) { return _pdep_u64(hw, kMask5555); }

HEADER_INLINE uintptr_t UnpackHalfwordToWordShift1(uintptr_t hw) { return _pdep_u64(hw, kMaskAAAA); }

HEADER_INLINE Vec4thUint UnpackVec8thUintTo4th(Vec8thUint hw) { return _pdep_u64(hw, kMask5555); }

HEADER_INLINE Halfword PackWordToHalfword(uintptr_t ww) { return _pext_u64(ww, kMask5555); }

HEADER_INLINE Halfword PackWordToHalfwordMask5555(uintptr_t ww) { return _pext_u64(ww, kMask5555); }

HEADER_INLINE Halfword PackWordToHalfwordMaskAAAA(uintptr_t ww) { return _pext_u64(ww, kMaskAAAA); }
...
HEADER_INLINE Vec8thUint PackVec4thUintTo8th(Vec4thUint ww) { return _pext_u64(ww, kMask5555); }

HEADER_INLINE Vec16thUint PackVec8thUintTo16th(Vec8thUint ww) { return _pext_u64(ww, kMask5555); }

HEADER_INLINE Halfword Pack3333(uintptr_t ww) { return _pext_u64(ww, kMask3333); }

HEADER_INLINE Halfword Pack3333Mask(uintptr_t ww) { return _pext_u64(ww, kMask3333); }
...
HEADER_INLINE uintptr_t Unpack0F0F(uintptr_t hw) { return _pdep_u64(hw, kMask0F0F); }

HEADER_INLINE Halfword Pack0F0F(uintptr_t ww) { return _pext_u64(ww, kMask0F0F); }

HEADER_INLINE Halfword Pack0F0FMask(uintptr_t ww) { return _pext_u64(ww, kMask0F0F); }
...
HEADER_INLINE uintptr_t PackTwo0F0FMask(uintptr_t lo, uintptr_t hi) { return PackTwo0F0F(lo, hi); }

HEADER_INLINE uintptr_t Unpack0303(uintptr_t qw) { return _pdep_u64(qw, kMask0303); }

HEADER_INLINE Quarterword Pack0303(uintptr_t ww) { return _pext_u64(ww, kMask0303); }

HEADER_INLINE Quarterword Pack0303Mask(uintptr_t ww) { return _pext_u64(ww, kMask0303); }
...
HEADER_INLINE uintptr_t UnpackHalfwordToWordShift1(uintptr_t hw) { return UnpackHalfwordToWord(hw) << 1; }
...
HEADER_INLINE Halfword PackWordToHalfwordMask5555(uintptr_t ww) { return PackWordToHalfword(ww & kMask5555); }

HEADER_INLINE Halfword PackWordToHalfwordMaskAAAA(uintptr_t ww) { return PackWordToHalfword((ww >> 1) & kMask5555); }
...
HEADER_INLINE Halfword Pack3333Mask(uintptr_t ww) { return Pack3333(ww & kMask3333); }
...
HEADER_INLINE Halfword Pack0F0FMask(uintptr_t ww) { return Pack0F0F(ww & kMask0F0F); }
...
HEADER_INLINE uintptr_t Pack0303Mask(uintptr_t ww) { return Pack0303(ww & kMask0303); }
...
HEADER_CINLINE uintptr_t RoundDownPow2(uintptr_t val, uintptr_t alignment) { return val & (~(alignment - 1)); }

HEADER_CINLINE uint64_t RoundDownPow2U64(uint64_t val, uint64_t alignment) { return val & (~(alignment - 1)); }

HEADER_CINLINE uintptr_t RoundUpPow2(uintptr_t val, uintptr_t alignment) { return (val + alignment - 1) & (~(alignment - 1)); }

HEADER_CINLINE uint64_t RoundUpPow2U64(uint64_t val, uint64_t alignment) { return (val + alignment - 1) & (~(alignment - 1)); }
...
HEADER_CINLINE uintptr_t DivUp(uintptr_t val, uint32_t divisor) { return (val + divisor - 1) / divisor; }

HEADER_CINLINE uint64_t DivUpU64(uint64_t val, uint32_t divisor) { return (val + divisor - 1) / divisor; }
...
HEADER_INLINE uint32_t ModNz(uintptr_t val, uint32_t modulus) { return (1 + ((val - 1) % modulus)); }
...
HEADER_INLINE uint32_t IsI32Neg(uint32_t uii) { return uii >> 31; }
...
HEADER_INLINE BoolErr ScanPosintCapped(const char *str, uint32_t cap, uint32_t *valp) { return ScanPosintCapped32(str, cap / 10, cap % 10, valp); }

HEADER_INLINE BoolErr ScanUintCapped(const char *str, uint32_t cap, uint32_t *valp) { return ScanUintCapped32(str, cap / 10, cap % 10, valp); }
...
HEADER_INLINE BoolErr ScanInt32(const char *str, int32_t *valp) { return ScanIntAbsBounded(str, 0x7fffffff, valp); }

// default cap = 0x7ffffffe
HEADER_INLINE BoolErr ScanPosintDefcap(const char *str, uint32_t *valp) { return ScanPosintCapped(str, 0x7ffffffe, valp); }

HEADER_INLINE BoolErr ScanUintDefcap(const char *str, uint32_t *valp) { return ScanUintCapped(str, 0x7ffffffe, valp); }

HEADER_INLINE BoolErr ScanIntAbsDefcap(const char *str, int32_t *valp) { return ScanIntAbsBounded(str, 0x7ffffffe, valp); }

HEADER_INLINE BoolErr ScanUintIcap(const char *str, uint32_t *valp) { return ScanUintCapped(str, 0x7fffffff, valp); }
...
HEADER_CINLINE uintptr_t BitCtToVecCt(uintptr_t val) { return DivUp(val, kBitsPerVec); }

HEADER_CINLINE uintptr_t BitCtToWordCt(uintptr_t val) { return DivUp(val, kBitsPerWord); }

HEADER_CINLINE uintptr_t BitCtToAlignedWordCt(uintptr_t val) { return kWordsPerVec * BitCtToVecCt(val); }

HEADER_CINLINE uintptr_t BitCtToCachelineCt(uintptr_t val) { return DivUp(val, kBitsPerCacheline); }

HEADER_CINLINE uintptr_t Int32CtToVecCt(uintptr_t val) { return DivUp(val, kInt32PerVec); }

HEADER_CINLINE uintptr_t Int32CtToCachelineCt(uintptr_t val) { return DivUp(val, kInt32PerCacheline); }

HEADER_CINLINE uintptr_t WordCtToVecCt(uintptr_t val) { return DivUp(val, kWordsPerVec); }

HEADER_CINLINE uintptr_t WordCtToCachelineCt(uintptr_t val) { return DivUp(val, kWordsPerCacheline); }

HEADER_CINLINE uint64_t WordCtToCachelineCtU64(uint64_t val) { return DivUpU64(val, kWordsPerCacheline); }

#ifdef __LP64__
HEADER_CINLINE uintptr_t Int64CtToVecCt(uintptr_t val) { return DivUp(val, kBytesPerVec / 8); }
#else
HEADER_CINLINE uintptr_t Int64CtToVecCt(uintptr_t val) { return val * 2; }
#endif

HEADER_CINLINE uintptr_t Int64CtToCachelineCt(uintptr_t val) { return DivUp(val, kInt64PerCacheline); }

HEADER_CINLINE uintptr_t DblCtToVecCt(uintptr_t val) { return Int64CtToVecCt(val); }

HEADER_CINLINE uintptr_t VecCtToCachelineCt(uintptr_t val) { return DivUp(val, kVecsPerCacheline); }

HEADER_CINLINE uint64_t VecCtToCachelineCtU64(uint64_t val) { return DivUpU64(val, kVecsPerCacheline); }
...
#ifdef USE_SSE42
HEADER_CINLINE uint32_t NypsumWord(uintptr_t val) { return __builtin_popcountll(val) + __builtin_popcountll(val & kMaskAAAA); }
#else
...
#ifdef USE_SSE42
HEADER_CINLINE uint32_t PopcountWord(uintptr_t val) { return __builtin_popcountll(val); }
#else
...
#ifdef USE_SSE42
HEADER_INLINE uint32_t Popcount2Words(uintptr_t val0, uintptr_t val1) { return __builtin_popcountll(val0) + __builtin_popcountll(val1); }
#else
...
HEADER_INLINE uint32_t Popcount4Words(uintptr_t val0, uintptr_t val1, uintptr_t val2, uintptr_t val3) {
    ...
    return (((four_bit_0 & kMask0F0F) + ((four_bit_0 >> 4) & kMask0F0F) + (four_bit_1 & kMask0F0F) + ((four_bit_1 >> 4) & kMask0F0F)) * kMask0101) >>
           (kBitsPerWord - 8);
}
#endif

#ifdef USE_SSE42
HEADER_INLINE uint32_t PopcountHW(uint32_t val) { return __builtin_popcount(val); }

HEADER_INLINE uint32_t PopcountByte(uint32_t val) { return __builtin_popcount(val); }
#else
#ifdef __LP64__
...
#ifdef USE_SSE2
#ifdef USE_SSE42
HEADER_INLINE uint32_t PopcountVec8thUint(uint32_t val) { return __builtin_popcount(val); }
#else
...
HEADER_INLINE uint64_t _vec0255_is_nonzero(VecUc vv) { return vecuc_movemask(vv); }

HEADER_INLINE uint32_t vec0255_set_ct(VecUc vv) { return PopcountVec8thUint(vecuc_movemask(vv)); }

HEADER_INLINE uint32_t vec0255_is_all_set(VecUc vv) { return (vecuc_movemask(vv) == kVec8thUintMax); }

HEADER_INLINE uint32_t vec0255u16_is_all_set(VecU16 vv) { return (vecu16_movemask(vv) == kVec8thUintMax); }

HEADER_INLINE uint32_t m128is_are_equal(__m128i v1, __m128i v2) { return (_mm_movemask_epi8(_mm_cmpeq_epi8(v1, v2)) == 65535); }
#else
...
HEADER_INLINE uint64_t _vec0255_is_nonzero(VecUc vv) { return arm_shrn4_uc(vv); }
...
HEADER_INLINE uint32_t count_set_nybbles(uint64_t xx) { return popcount_bits4(xx & kMask1111); }

HEADER_INLINE uint32_t vec0255_set_ct(VecUc vv) { return count_set_nybbles(arm_shrn4_uc(vv)); }

HEADER_INLINE uint32_t vec0255_is_all_set(VecUc vv) { return (arm_shrn4_uc(vv) == UINT64_MAX); }

HEADER_INLINE uint32_t vec0255u16_is_all_set(VecU16 vv) { return (arm_shrn4_u16(vv) == UINT64_MAX); }
...
HEADER_INLINE uint32_t vecucs_are_equal(VecUc v1, VecUc v2) { return vec0255_is_all_set(v1 == v2); }

HEADER_INLINE uint32_t vecu16s_are_equal(VecU16 v1, VecU16 v2) { return vec0255u16_is_all_set(v1 == v2); }
#endif

// Downcasts don't risk alignment issues.
HEADER_INLINE unsigned char *DowncastToUc(void *pp) { return S_CAST(unsigned char *, pp); }

HEADER_INLINE char *DowncastToC(void *pp) { return S_CAST(char *, pp); }

HEADER_INLINE const unsigned char *DowncastKToUc(const void *pp) { return S_CAST(const unsigned char *, pp); }

HEADER_INLINE const char *DowncastKToC(const void *pp) { return S_CAST(const char *, pp); }

HEADER_INLINE uintptr_t *DowncastVecWToW(VecW *pp) { return R_CAST(uintptr_t *, pp); }

HEADER_INLINE uint32_t *DowncastVecWToU32(VecW *pp) { return R_CAST(uint32_t *, pp); }

HEADER_INLINE Halfword *DowncastWToHW(uintptr_t *pp) { return R_CAST(Halfword *, pp); }

HEADER_INLINE uint32_t *DowncastWToU32(uintptr_t *pp) { return R_CAST(uint32_t *, pp); }

HEADER_INLINE uint16_t *DowncastWToU16(uintptr_t *pp) { return R_CAST(uint16_t *, pp); }

HEADER_INLINE int16_t *DowncastWToI16(uintptr_t *pp) { return R_CAST(int16_t *, pp); }

#ifdef USE_SSE2
HEADER_INLINE Vec8thUint *DowncastWToV8(uintptr_t *pp) { return R_CAST(Vec8thUint *, pp); }
#endif

HEADER_INLINE uint16_t *DowncastU32ToU16(uint32_t *pp) { return R_CAST(uint16_t *, pp); }

HEADER_INLINE const uintptr_t *DowncastKVecWToW(const VecW *pp) { return R_CAST(const uintptr_t *, pp); }

HEADER_INLINE const uint16_t *DowncastKVecWToU16(const VecW *pp) { return R_CAST(const uint16_t *, pp); }

HEADER_INLINE const Halfword *DowncastKWToHW(const uintptr_t *pp) { return R_CAST(const Halfword *, pp); }

HEADER_INLINE const uint32_t *DowncastKWToU32(const uintptr_t *pp) { return R_CAST(const uint32_t *, pp); }

HEADER_INLINE const uint16_t *DowncastKWToU16(const uintptr_t *pp) { return R_CAST(const uint16_t *, pp); }

HEADER_INLINE const uint16_t *DowncastKU64ToU16(const uint64_t *pp) { return R_CAST(const uint16_t *, pp); }

HEADER_INLINE uint32_t IsVecAligned(const void *ptr) { return !(R_CAST(uintptr_t, ptr) % kBytesPerVec); }
...
#ifdef __LP64__
HEADER_INLINE Halfword SubHWLoad(const void *bytearr, uint32_t ct) { return SubU32Load(bytearr, ct); }
#else
HEADER_INLINE Halfword SubHWLoad(const void *bytearr, uint32_t ct) { return SubU16Load(bytearr, ct); }
#endif
...
HEADER_INLINE void ProperSubwordStore(uintptr_t cur_word, uint32_t byte_ct, void *target) { memcpy(target, &cur_word, byte_ct); }

HEADER_INLINE void SubwordStore(uintptr_t cur_word, uint32_t byte_ct, void *target) { memcpy(target, &cur_word, byte_ct); }

HEADER_INLINE void SubU32Store(uint32_t cur_uint, uint32_t byte_ct, void *target) { memcpy(target, &cur_uint, byte_ct); }
#endif // NO_UNALIGNED
...
#ifndef NO_UNALIGNED
#ifdef __LP64__
HEADER_INLINE void SubU64StoreMov(uint64_t cur_u64, uint32_t byte_ct, unsigned char **targetp) { return SubwordStoreMov(cur_u64, byte_ct, targetp); }
#else
...
HEADER_INLINE BoolErr cachealigned_malloc(uintptr_t size, void *aligned_pp) { return aligned_malloc(size, kCacheline, aligned_pp); }

HEADER_INLINE void aligned_free(void *aligned_ptr) { free(R_CAST(void *, S_CAST(uintptr_t *, aligned_ptr)[-1])); }
...
HEADER_INLINE void free_const(const void *memptr) { free(K_CAST(void *, memptr)); }
...
#ifdef USE_AVX2
HEADER_INLINE void vecaligned_free(void *aligned_ptr) { aligned_free(aligned_ptr); }

HEADER_INLINE void vecaligned_free_cond(void *aligned_ptr) { aligned_free_cond(aligned_ptr); }
#else
#if defined(__APPLE__) || !defined(__LP64__)
HEADER_INLINE void vecaligned_free(void *aligned_ptr) { free(aligned_ptr); }

HEADER_INLINE void vecaligned_free_cond(void *aligned_ptr) { free_cond(aligned_ptr); }
#else
HEADER_INLINE void vecaligned_free(void *aligned_ptr) { aligned_free(aligned_ptr); }
...
#else
HEADER_INLINE int32_t memequal(const void *m1, const void *m2, uintptr_t byte_ct) { return !memcmp(m1, m2, byte_ct); }

HEADER_INLINE int32_t Memcmp(const void *m1, const void *m2, uintptr_t ct) { return memcmp(m1, m2, ct); }
#endif
...
template <uint32_t N, typename = TRange<true>> struct MemequalKImpl {
    static int32_t MemequalK(const void *m1, const void *m2) { return memequal(m1, m2, N); }
};
...
template <> struct MemequalKImpl<2> {
    static int32_t MemequalK(const void *m1, const void *m2) { return ((*R_CAST(const uint16_t *, m1)) == (*R_CAST(const uint16_t *, m2))); }
};
...
template <> struct MemequalKImpl<3> {
    static int32_t MemequalK(const void *m1, const void *m2) {
        const unsigned char *m1_uc = S_CAST(const unsigned char *, m1);
        const unsigned char *m2_uc = S_CAST(const unsigned char *, m2);
        return ((*R_CAST(const uint16_t *, m1)) == (*R_CAST(const uint16_t *, m2))) && (m1_uc[2] == m2_uc[2]);
    }
};

template <> struct MemequalKImpl<4> {
    static int32_t MemequalK(const void *m1, const void *m2) { return ((*R_CAST(const uint32_t *, m1)) == (*R_CAST(const uint32_t *, m2))); }
};
...
template <> struct MemequalKImpl<8> {
    static int32_t MemequalK(const void *m1, const void *m2) { return ((*R_CAST(const uint64_t *, m1)) == (*R_CAST(const uint64_t *, m2))); }
};
...
template <uint32_t N> struct MemequalKImpl<N, TRange<(17 <= N) && (N <= 24)>> {
    static int32_t MemequalK(const void *m1, const void *m2) {
        ...
        const __m128i v1 = _mm_loadu_si128(S_CAST(const __m128i *, m1));
        const __m128i v2 = _mm_loadu_si128(S_CAST(const __m128i *, m2));
        return m128is_are_equal(v1, v2) && ((*R_CAST(const uint64_t *, &(m1_uc[N - 8]))) == (*R_CAST(const uint64_t *, &(m2_uc[N - 8]))));
    }
};
...
template <uint32_t N, typename = TRange<true>> struct MemcpyKImpl {
    static void MemcpyK(void *__restrict dst, const void *__restrict src) { memcpy(dst, src, N); }
};
...
template <> struct MemcpyKImpl<2> {
    static void MemcpyK(void *__restrict dst, const void *__restrict src) { *S_CAST(uint16_t *, dst) = *S_CAST(const uint16_t *, src); }
};
...
template <uint32_t N> struct MemcpyoKImpl {
    static void MemcpyoK(void *__restrict dst, const void *__restrict src) { MemcpyKImpl<N>::MemcpyK(dst, src); }
};

template <> struct MemcpyoKImpl<3> {
    static void MemcpyoK(void *__restrict dst, const void *__restrict src) { *S_CAST(uint32_t *, dst) = *S_CAST(const uint32_t *, src); }
};

template <> struct MemcpyoKImpl<7> {
    static void MemcpyoK(void *__restrict dst, const void *__restrict src) { *S_CAST(uint64_t *, dst) = *S_CAST(const uint64_t *, src); }
};
...
HEADER_INLINE int32_t memequal_k(const void *m1, const void *m2, uintptr_t ct) { return !memcmp(m1, m2, ct); }

HEADER_INLINE void memcpy_k(void *__restrict dst, const void *__restrict src, uintptr_t ct) { memcpy(dst, src, ct); }

HEADER_INLINE char *memcpya_k(void *__restrict dst, const void *__restrict src, uintptr_t ct) { return memcpya(dst, src, ct); }

HEADER_INLINE unsigned char *memcpyua_k(void *__restrict dst, const void *__restrict src, uintptr_t ct) { return memcpyua(dst, src, ct); }

HEADER_INLINE void memcpyo_k(void *__restrict dst, const void *__restrict src, uintptr_t ct) { memcpy(dst, src, ct); }

HEADER_INLINE char *memcpyao_k(void *__restrict dst, const void *__restrict src, uintptr_t ct) { return memcpya(dst, src, ct); }

HEADER_INLINE unsigned char *memcpyuao_k(void *__restrict dst, const void *__restrict src, uintptr_t ct) { return memcpyua(dst, src, ct); }
...
constexpr uint32_t CompileTimeSlen(const char *k_str) { return k_str[0] ? (1 + CompileTimeSlen(&(k_str[1]))) : 0; }
#endif
...
HEADER_INLINE void strcpy_k(char *__restrict dst, const void *__restrict src) { strcpy(dst, S_CAST(const char *, src)); }

HEADER_INLINE char *strcpya_k(char *__restrict dst, const void *__restrict src) { return strcpya(dst, src); }
...
HEADER_INLINE char *wtoa(uintptr_t ulii, char *start) { return i64toa(ulii, start); }
#else
HEADER_INLINE char *wtoa(uintptr_t ulii, char *start) { return u32toa(ulii, start); }
#endif

HEADER_INLINE void CopyFromUnalignedW(uintptr_t *dst, const unsigned char *src) { memcpy_k(dst, src, kBytesPerWord); }

HEADER_INLINE void CopyFromUnalignedU32(uint32_t *dst, const unsigned char *src) { memcpy_k(dst, src, sizeof(int32_t)); }

HEADER_INLINE void CopyFromUnalignedU16(uint16_t *dst, const unsigned char *src) { memcpy_k(dst, src, sizeof(int16_t)); }

HEADER_INLINE void CopyFromUnalignedI16(int16_t *dst, const unsigned char *src) { memcpy_k(dst, src, sizeof(int16_t)); }

HEADER_INLINE void CopyFromUnalignedF(float *dst, const unsigned char *src) { memcpy_k(dst, src, sizeof(float)); }
...
HEADER_INLINE void CopyToUnalignedW(unsigned char *dst, const uintptr_t *src) { memcpy_k(dst, src, kBytesPerWord); }

HEADER_INLINE void CopyToUnalignedU64(unsigned char *dst, const uint64_t *src) { memcpy_k(dst, src, sizeof(int64_t)); }

HEADER_INLINE void CopyToUnalignedU32(unsigned char *dst, const uint32_t *src) { memcpy_k(dst, src, sizeof(int32_t)); }

HEADER_INLINE void CopyToUnalignedF(unsigned char *dst, const float *src) { memcpy_k(dst, src, sizeof(float)); }
...
HEADER_INLINE void ZeroU32Arr(uintptr_t entry_ct, uint32_t *u32arr) { memset(u32arr, 0, entry_ct * sizeof(int32_t)); }

HEADER_INLINE void ZeroWArr(uintptr_t entry_ct, uintptr_t *warr) { memset(warr, 0, entry_ct * sizeof(intptr_t)); }

HEADER_INLINE void ZeroU64Arr(uintptr_t entry_ct, uint64_t *u64arr) { memset(u64arr, 0, entry_ct * sizeof(int64_t)); }

HEADER_INLINE void ZeroPtrArr(uintptr_t entry_ct, void *pp) { memset(pp, 0, entry_ct * sizeof(intptr_t)); }

HEADER_INLINE void ZeroHwArr(uintptr_t entry_ct, Halfword *hwarr) { memset(hwarr, 0, entry_ct * sizeof(Halfword)); }
...
HEADER_INLINE uintptr_t bzhi(uintptr_t ww, uint32_t idx) { return ww & ((k1LU << idx) - k1LU); }

// This is undefined if idx == 0.
HEADER_INLINE uintptr_t bzhi_max(uintptr_t ww, uint32_t idx) { return ww & ((~k0LU) >> (kBitsPerWord - idx)); }
...
HEADER_INLINE uint32_t BytesToRepresentNzU32(uint32_t uii) { return 1 + (bsru32(uii) / CHAR_BIT); }
...
HEADER_INLINE uint32_t WordBitIdxToUidx(uintptr_t ulii, uint32_t bit_idx) { return ctzw(ClearBottomSetBits(bit_idx, ulii)); }

CONSTI32(kNybblesPerWord, 2 * kBytesPerWord);
CONSTI32(kNybblesPerCacheline, 2 * kCacheline);

HEADER_CINLINE uintptr_t NybbleCtToByteCt(uintptr_t val) { return DivUp(val, 2); }

HEADER_CINLINE uintptr_t NybbleCtToWordCt(uintptr_t val) { return DivUp(val, kNybblesPerWord); }
...
HEADER_INLINE uintptr_t DetectFirstZeroByte(uintptr_t ww) { return (ww - kMask0101) & (~ww) & (kMask0101 * 0x80); }

// From TAOCP 4a, 7.1.3, (91).
// Position 8x + 7 is always set iff byte x is zero.  All other bits are always
// zero.
HEADER_INLINE uintptr_t DetectAllZeroBytes(uintptr_t ww) { return (kMask0101 * 0x80) & (~(ww | ((ww | (kMask0101 * 0x80)) - kMask0101))); }

HEADER_INLINE uintptr_t DetectAllZeroNybbles(uintptr_t ww) { return (kMask1111 * 8) & (~(ww | ((ww | (kMask1111 * 8)) - kMask1111))); }
...
HEADER_INLINE uintptr_t CountVintsNonempty(const unsigned char *buf, const unsigned char *buf_end) { return CountVints(buf, buf_end); }
#endif
...
#define FLAGSET_DEF_END(tname)                                                                                                                       \
    ...
    tname##_PLINK2_BASE_DO_NOT_USE__;                                                                                                                \
                                                                                                                                                     \
    ...
    struct tname {                                                                                                                                   \
        ...
        explicit tname(uint32_t source) : value_(source) {}                                                                                          \
                                                                                                                                                     \
        operator tname##_PLINK2_BASE_DO_NOT_USE__() const { return static_cast<tname##_PLINK2_BASE_DO_NOT_USE__>(value_); }                          \
                                                                                                                                                     \
...
#define FLAGSET64_DEF_END(tname)                                                                                                                     \
    , tname##PLINK2_BASE_DO_NOT_USE__ALL_64_SET__ = ~(0LLU)                                                                                          \
    }                                                                                                                                                \
    tname##_PLINK2_BASE_DO_NOT_USE__;                                                                                                                \
