...
static inline textFILEMain *GetTxfp(textFILE *txf_ptr) { return &GET_PRIVATE(*txf_ptr, m); }

static inline TextStreamMain *GetTxsp(TextStream *txs_ptr) { return &GET_PRIVATE(*txs_ptr, m); }

static inline const TextStreamMain *GetTxspK(const TextStream *txs_ptr) { return &GET_PRIVATE(*txs_ptr, m); }
...
void PreinitTextFile(textFILE *txf_ptr) { EraseTextFileBase(&GetTxfp(txf_ptr)->base); }
...
const char kShortErrRfileEnforcedMaxBlenTooSmall[] =
    "TextFileOpenInternal: enforced_max_line_blen too small (must be at least max(1 MiB, dst_capacity - 1 MiB))";
const char kShortErrRfileDstCapacityTooSmall[] = "TextFileOpenInternal: dst_capacity too small (2 MiB minimum)";

PglErr TextFileOpenInternal(const char *fname, uint32_t enforced_max_line_blen, uint32_t dst_capacity, char *dst, textFILEMain *txfp,
                            TextStreamMain *txsp) {
...
PglErr TextStreamOpenEx(const char *fname, uint32_t enforced_max_line_blen, uint32_t dst_capacity, uint32_t decompress_thread_ct, textFILE *txf_ptr,
                        char *dst, TextStream *txs_ptr) {
    TextStreamMain *txsp = GetTxsp(txs_ptr);
...
#ifdef __cplusplus
        if(unlikely(pthread_create(&syncp->read_thread, &g_thread_startup.smallstack_thread_attr, TextStreamThread, txsp))) {
            goto TextStreamOpenEx_ret_THREAD_CREATE_FAIL;
        }
#else
...
PglErr TextAdvance(TextStream *txs_ptr) {
    ...
    while(1) {
        ...
        // printf("checking for more to consume: %lx %lx %lx\n", (uintptr_t)consume_iter, (uintptr_t)syncp->cur_circular_end,
