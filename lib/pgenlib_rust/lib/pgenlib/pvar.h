...
struct classcomp {
    bool operator()(const char *const &lhs, const char *const &rhs) const { return strcmp(lhs, rhs) < 0; }
};
...
class RPvar {
  public:
    ...
    void Load(std::string filename, bool omit_chrom, bool omit_pos);
...
// HEADER_INLINE int strcmp_r_c(String r_string, const char *cstr) { return strcmp(r_string.get_cstring(), cstr); }
